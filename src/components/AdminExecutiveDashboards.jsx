  ...
  return <>
    ...
    {view === 'sales' && <section className="executive-dashboard-view">
      ...
      <div className="executive-kpis">
        <Kpi icon={<Sprout />} label="Coste total de las ventas" value={money(data.totalCost)} detail={`${data.costedUnits} túperes con coste trazado · ${data.costCoverage.toFixed(1)} % de cobertura`} tone="amber" onClick={() => openFinancial('profit-products')} />
        ...
        <Kpi icon={<TrendingUp />} label="Margen de beneficio" value={`${data.marginPercent.toFixed(1)} %`} detail={`${money(data.tracedRevenue)} de venta con costes vinculados`} tone="purple" onClick={() => openFinancial('profit-products')} />
      ...
      <div className="executive-charts executive-sales-charts">
        <article className="executive-chart chart-green"><header><div><h3>Ventas y coste de lo vendido</h3><p>Coste completo trazado de los táperes entregados</p></div><strong>{money(data.monthSales)}</strong></header><div>{data.chart.some(row => row.Ventas > 0 || row['Coste vendido'] > 0) ? <ResponsiveContainer><BarChart data={data.chart}><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name"/><YAxis tickFormatter={value => `${value} €`}/><Tooltip formatter={money}/><Legend iconType="circle" iconSize={8}/><Bar dataKey="Ventas" fill="#10b981" radius={[5,5,0,0]}/><Bar dataKey="Coste vendido" fill="#f59e0b" radius={[5,5,0,0]}/></BarChart></ResponsiveContainer> : <Empty>Sin ventas ni costes trazados en este periodo.</Empty>}</div></article>
    ...
    {view === 'profitability' && <section className="executive-dashboard-view">
      ...
      <div className="executive-kpis">
        ...
        <Kpi icon={<PackageCheck />} label="Coste de esas ventas" value={money(data.totalCost)} detail={`${data.costedUnits} unidades con coste vinculado`} tone="blue" onClick={() => openFinancial('profit-products')} />
        <Kpi icon={<TrendingUp />} label="Beneficio bruto trazado" value={money(data.margin)} detail={`${data.marginPercent.toFixed(1)} % de margen`} onClick={() => openFinancial('profit-products')} />
