...
export default function AdminExecutiveDashboards({ data, view, onViewChange, openFinancial, openOrders }) {
  ...
  return <>
    <nav className="executive-dashboard-tabs" aria-label="Dashboards de administración">
      <button className={view === 'sales' ? 'active' : ''} onClick={() => onViewChange('sales')}><ShoppingBag /><span><strong>Ventas</strong><small>Día a día y pedidos</small></span></button>
      <button className={view === 'production' ? 'active' : ''} onClick={() => onViewChange('production')}><Sprout /><span><strong>Producción</strong><small>Cultivos y costes</small></span></button>
      <button className={view === 'profitability' ? 'active' : ''} onClick={() => onViewChange('profitability')}><TrendingUp /><span><strong>Rentabilidad general</strong><small>Resultado de la empresa</small></span></button>
    </nav>
    ...
    {view === 'sales' && <section className="executive-dashboard-view">
      ...
      <div className="executive-kpis">
        ...
        <Kpi icon={<PackageCheck />} label="Unidades vendidas" value={data.units} detail={`${money(data.averageTicket)} por pedido`} tone="blue" onClick={() => openFinancial('products')} />
      ...
      <div className="executive-charts executive-sales-charts">
        <article className="executive-chart executive-chart-wide"><header><div><h3>Evolución de ventas</h3><p>Importe entregado por fecha</p></div><strong>{money(data.monthSales)}</strong></header><div>{data.chart.some(row => row.Ventas > 0) ? <ResponsiveContainer><AreaChart data={data.chart}><defs><linearGradient id="executiveSales" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity=".35"/><stop offset="95%" stopColor="#10b981" stopOpacity="0"/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name"/><YAxis tickFormatter={value => `${value} €`}/><Tooltip formatter={money}/><Area type="monotone" dataKey="Ventas" stroke="#10b981" strokeWidth={3} fill="url(#executiveSales)"/></AreaChart></ResponsiveContainer> : <Empty>Sin ventas en este periodo.</Empty>}</div></article>
        <article className="executive-chart"><header><div><h3>Productos más vendidos</h3><p>Facturación por producto</p></div></header><div>{topProducts.length ? <ResponsiveContainer><BarChart data={topProducts} layout="vertical"><XAxis type="number" hide/><YAxis type="category" dataKey="shortName" width={115}/><Tooltip formatter={money}/><Bar dataKey="total" name="Ventas" fill="#0ea5e9" radius={[0,7,7,0]}/></BarChart></ResponsiveContainer> : <Empty>Sin productos vendidos.</Empty>}</div></article>
        <article className="executive-chart"><header><div><h3>Clientes principales</h3><p>Facturación por cliente</p></div></header><div>{topClients.length ? <ResponsiveContainer><BarChart data={topClients} layout="vertical"><XAxis type="number" hide/><YAxis type="category" dataKey="shortName" width={115}/><Tooltip formatter={money}/><Bar dataKey="total" name="Ventas" fill="#8b5cf6" radius={[0,7,7,0]}/></BarChart></ResponsiveContainer> : <Empty>Sin clientes con ventas.</Empty>}</div></article>
      </div>
    </section>}
    ...
    {view === 'production' && <section className="executive-dashboard-view">
      ...
      <div className="executive-charts">
        <article className="executive-chart"><header><div><h3>Distribución del coste</h3><p>Materiales y envasado</p></div></header><div>{data.productionBreakdown.some(row => row.value > 0) ? <ResponsiveContainer><PieChart><Pie data={data.productionBreakdown.filter(row => row.value > 0)} dataKey="value" nameKey="name" innerRadius={58} outerRadius={90} paddingAngle={3}>{data.productionBreakdown.filter(row => row.value > 0).map((row,index)=><Cell key={row.name} fill={colors[index%colors.length]}/>)}</Pie><Tooltip formatter={money}/><Legend/></PieChart></ResponsiveContainer> : <Empty>Sin costes productivos en el periodo.</Empty>}</div></article>
        <article className="executive-chart"><header><div><h3>Cultivos en marcha</h3><p>Coste previsto por variedad</p></div></header><div>{productionVarieties.length ? <ResponsiveContainer><BarChart data={productionVarieties} layout="vertical"><XAxis type="number" hide/><YAxis type="category" dataKey="shortName" width={115}/><Tooltip formatter={money}/><Bar dataKey="cost" name="Coste" fill="#f59e0b" radius={[0,7,7,0]}/></BarChart></ResponsiveContainer> : <Empty>No hay cosechas previstas en el periodo.</Empty>}</div></article>
        <article className="executive-chart"><header><div><h3>Producción terminada</h3><p>Coste real por producto cosechado</p></div></header><div>{harvestedProducts.length ? <ResponsiveContainer><BarChart data={harvestedProducts} layout="vertical"><XAxis type="number" hide/><YAxis type="category" dataKey="shortName" width={115}/><Tooltip formatter={money}/><Bar dataKey="cost" name="Coste" fill="#10b981" radius={[0,7,7,0]}/></BarChart></ResponsiveContainer> : <Empty>Sin cosechas terminadas en el periodo.</Empty>}</div></article>
      </div>
    </section>}
