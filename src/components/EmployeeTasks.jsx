          ...
          {groups.map(group => {
            ...
            return (
              <button key={group.key} type="button" disabled={disabled} onClick={() => {
                ...
                <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem' }}><strong style={{ color: disabled ? '#64748b' : group.color, fontSize: '1rem' }}>{group.icon} {group.title}</strong><span style={{ minWidth: '46px', height: '46px', padding: '0 0.55rem', display: 'grid', placeItems: 'center', borderRadius: '999px', background: disabled ? '#e2e8f0' : 'white', color: disabled ? '#64748b' : group.color, fontSize: '1.35rem', lineHeight: 1, fontWeight: 950, boxShadow: '0 2px 8px rgba(15, 23, 42, 0.08)' }}>{group.items.length}</span></span>
